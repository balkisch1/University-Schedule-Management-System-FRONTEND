...
updatePasswordFormGroup!: FormGroup;
  ...
  ngOnInit() {
    ...
    console.log('User ID from cookie:', id);  // Ajouter un log pour vérifier l'ID récupéré
    this.getProf(id);
  ...
  getProf(id: number) {
    this.profService.getProf(id).subscribe(
      (prof: Prof) => {
        this.prof = prof;
        console.log('Prof data received:', prof);  // Vérifier les données de prof
      },
      (error) => {
        console.error('Error fetching prof data:', error);  // Enregistrer l'erreur
      }
    );
  }
